...
var getMoneyAmount = function (n) {
  ...
  const costCache = new Array(n + 1).fill(0).map(() => new Array(n + 1));
  const cost = (low, high) => {
    if (low >= high) {
      return 0;
    }
    const cached = costCache[low][high];
    if (cached !== undefined) {
      return cached;
    }
    let minres = Number.MAX_SAFE_INTEGER;
    for (let i = low; i <= high; i++) {
      const res = i + Math.max(cost(low, i - 1), cost(i + 1, high));
      minres = Math.min(minres, res);
    }
    costCache[low][high] = minres;
    return minres;
  };
